...
2. **Multi-file write rule**: if implementation will touch 2+ non-trivial files, delegate one writer; inline writing is allowed only for trivial/mechanical edits. Any review work remains inside the already-bound transaction budget.
   ...
   Never open a visible project pane as an automatic delegation, retry, remediation, verification, or cross-worktree fallback. Preserve the selected execution surface and return its actionable failure instead. An explicit persistent peer conversation requires the owner to run `/gentle:allow-project-pane-once <cwd>` first; it authorizes exactly one matching intercom send or ask in that owner session.
