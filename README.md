...
Native review mode and the two candidate choices remain provider-owned lifecycle semantics. For a validated `consent/v3` envelope in the interactive parent TUI, Pi displays those two choices unchanged and adds a clearly separate host-owned action: **Run this review and allow reviews for this Pi session**. Only direct human selection creates this process-memory grant. It runs the current envelope's exact provider `granted` invocation through the existing one-shot `answer-consent` path, then does the same for each later fresh validated envelope bound to the exact live SessionManager object, nonempty session ID, and canonical Git worktree. Reload preserves it; `/tree` retains it; quit, new, resume, fork, process restart, or `/gentle:review-session-permission revoke` removes it. The command's `status` action reports the in-memory state without changing provider mode or authority.

The host grant is held only in a schema-checked `globalThis[Symbol.for(...)]` WeakMap registry. It is never written through session entries, settings, environment variables, or the old asked latch. Child-agent RPC processes, headless/RPC/unsupported UI, explicit cross-repository `workspaceRoot`, model prose, tool arguments, cancellation, identity drift, and uncertain native results cannot create or consume it. It grants no review verdict, forecast/cost approval, acknowledgement, maintenance, delivery, or cross-repository authority. When the host cannot resolve the choice, `gentle_review` returns the original unresolved two-choice provider envelope unchanged for the normal lossless relay. SessionManager binding isolates simultaneous SDK sessions; Pi does not claim universal same-process agent-principal isolation because the SDK exposes no principal identity.
