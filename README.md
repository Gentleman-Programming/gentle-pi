...
- A configured child can call `subagent_parent_message` with bounded, well-formed Unicode text. Its parent admits a notification only while the originating session is active; acceptance is not a delivery, read, or completion guarantee. A live child suppresses duplicate IDs only within its 64 most-recent acknowledgements: an evicted ID can be admitted again, so this is not lifetime exactly-once delivery. Channel closure prevents later sends and automatic retry is not provided. Queries, peer transport, offline delivery, retries, and broadcasts are unsupported in this slice.
