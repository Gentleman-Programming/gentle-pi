...
- `/gentle:agents` or `alt+a` opens the overlay: tasks on the left, the selected task's thread on the right. In fullscreen mode, hovering only highlights a task row; clicking selects it without opening its session or cancelling it; and the wheel scrolls the list or thread under the pointer independently. `j`/`k` move, `ctrl+j`/`ctrl+k` or `pgdn`/`pgup` scroll (`f` follows the tail again), `c` cancels, `o` opens a markdown transcript of the task's session in `$EDITOR` (written under `~/.pi/agent/gentle-agents/transcripts/`), `esc` closes. Only the selected task is subscribed while it is open.
