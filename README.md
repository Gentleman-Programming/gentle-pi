...
> **Fullscreen installation note:** a recognized global installation persists Pi’s `"tuiMode": "fullscreen"` setting only when no `tuiMode` is set; an explicit `"regular"` choice is preserved across updates. Project-local and other install paths do not receive that change.
