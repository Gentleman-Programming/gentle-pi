...
// v2.4.0 digests pinned from the published release: archive sha256 values match
...
const EXPECTED_ASSETS = {
	"darwin/amd64": { name: "gentle-ai_2.4.0_darwin_amd64.tar.gz", sha256: "c3f5a4251e927c41864e7a9a5670ac3fce97c6282755ec8e9b2e7e064067f750", binarySha256: "c4d25b98bae0d06b33baa23ba997cb64ab63c2e93e97983bcb9c239139bceaba" },
	"darwin/arm64": { name: "gentle-ai_2.4.0_darwin_arm64.tar.gz", sha256: "2240ae797119dd1dc5a0dd2fd2711077ce6712e66ab0b67461344c983478f7f4", binarySha256: "54a8a5957ce4f10f36b46ef818c888f484a3e213a71faca6b7783af6c029eb77" },
	"linux/amd64": { name: "gentle-ai_2.4.0_linux_amd64.tar.gz", sha256: "517eb8420cf966b74be1413a7ea1036119a5ccbb376e0778aa9f4d4d283e2e9a", binarySha256: "0be446737f8f1a9458fd2891d488beb2d72786578afead9bbf2d6ddddc9ec113" },
	"linux/arm64": { name: "gentle-ai_2.4.0_linux_arm64.tar.gz", sha256: "1badec9cad1f2bbeb22b5c6804c3a88c601d844abae231264e416e8095f2a15f", binarySha256: "cdd8fa43c3417e844cc11d2e436d4667edb0a54f086e0eb1d6cd3f7493bfee5a" },
} as const;
...
test("release mapping selects only the supported official v2.4.0 archive and pinned digests", () => {
	assert.deepEqual(
	...
	assert.equal(resolveGentleAiReleaseAsset("linux", "x64").name, "gentle-ai_2.4.0_linux_amd64.tar.gz");
	assert.equal(resolveGentleAiReleaseAsset("darwin", "arm64").name, "gentle-ai_2.4.0_darwin_arm64.tar.gz");
	for (const asset of Object.values(GENTLE_AI_RELEASE_ASSETS)) {
		assert.match(asset.url, /^https:\/\/github\.com\/Gentleman-Programming\/gentle-ai\/releases\/download\/v2\.4\.0\//);
	}
});
...
test("release digests are all-or-none and install fails closed while any digest is pending", async () => {
	...
	if (pending.length === digests.length) {
		...
		assert.equal(existsSync(join(packageRoot, ".gentle-ai", "v2.4.0", "gentle-ai")), false);
	}
});
...
function windowsGoFixture(fixtureOptions: WindowsGoFixtureOptions = {}) {
	...
	const metadata = [
		...
		`\tmod\tgithub.com/gentleman-programming/gentle-ai/v2\tv2.4.0\t${GENTLE_AI_WINDOWS_SOURCE_MODULE_CHECKSUM}`,
	...
	const run = async (file: string, arguments_: string[], options: WindowsGoCall["options"]) => {
		...
		if (arguments_.length === 1 && arguments_[0] === "version") return { stdout: fixtureOptions.reportedVersion ?? "gentle-ai 2.4.0\n", stderr: "" };
...
test("win32 x64 and arm64 install the exact Go SumDB source tag without archive lookup", async () => {
	for (const [arch, goArchitecture] of [["x64", "amd64"], ["arm64", "arm64"]] as const) {
		...
		assert.deepEqual(fixture.calls.filter((call) => call.file === goPath).map((call) => call.arguments_.slice(0, 2)), [
			["version"], ["install", "github.com/gentleman-programming/gentle-ai/v2/cmd/gentle-ai@v2.4.0"], ["version", "-m"],
		]);
	}
});
...
test("Windows source install reports missing or too-old Go without publishing a binary", async () => {
	for (const fixtureOptions of [
		...
		assert.equal(existsSync(join(packageRoot, ".gentle-ai", "v2.4.0", "gentle-ai.exe")), false);
...
test("Windows source install cleans staging after Go failure or wrong built version", async () => {
	for (const fixtureOptions of [
		...
		const runtimeDirectory = join(packageRoot, ".gentle-ai", "v2.4.0");
...
test("Windows source installs reuse only a fully verified package-local binary", async () => {
	...
	await writeFile(join(packageRoot, ".gentle-ai", "v2.4.0", "integrity.json"), "{}\n");
...
async function hardenedWindowsGoFixture(packageRoot: string, fixtureOptions: HardenedGoFixtureOptions = {}) {
	...
	const run = async (file: string, arguments_: string[], options: HardenedGoCall["options"]) => {
		...
		if (arguments_.length === 1 && arguments_[0] === "version") return { stdout: "gentle-ai 2.4.0\n", stderr: "" };
		throw new Error(`unexpected command: ${file} ${arguments_.join(" ")}`);
	};
	return {
		calls,
...
test("Windows source installation resolves one validated Go executable and seals hostile Go environment", async () => {
	...
	assert.deepEqual(goCalls.map((call) => call.arguments_.slice(0, 2)), [
		["version"],
		["install", "github.com/gentleman-programming/gentle-ai/v2/cmd/gentle-ai@v2.4.0"],
		["version", "-m"],
	]);
...
test("Windows source manifest binds verified Go metadata and architecture", async () => {
	...
	const manifest = JSON.parse(await readFile(join(packageRoot, ".gentle-ai", "v2.4.0", "integrity.json"), "utf8")) as Record<string, string>;
	assert.equal(result.installed, true);
...
test("Windows source installation treats a fresh ownerless lock as active", async () => {
	...
	const lockPath = join(packageRoot, ".gentle-ai", ".v2.4.0.install.lock");
...
test("Windows source installation preserves a lock whose owner nonce changed before release", async () => {
	...
	const lockOwnerPath = join(packageRoot, ".gentle-ai", ".v2.4.0.install.lock", "owner.json");
	const fixture = await hardenedWindowsGoFixture(packageRoot);
...
function tombstonePath(packageRoot: string, nonce: string): string {
	return join(packageRoot, ".gentle-ai", `.v2.4.0.install.tombstone-${nonce}`);
}
...
async function tombstones(packageRoot: string): Promise<string[]> {
	...
	return existsSync(runtimeRoot)
		? (await readdir(runtimeRoot)).filter((entry) => entry.startsWith(".v2.4.0.install.tombstone-"))
		: [];
}

function backupBundlePath(packageRoot: string, nonce: string): string {
	return join(packageRoot, ".gentle-ai", `.v2.4.0.backup-${nonce}`);
}
...
test("Windows source acquisition fails closed when a tombstone appears after its first scan", async () => {
	...
	assert.equal(existsSync(join(packageRoot, ".gentle-ai", ".v2.4.0.install.lock")), false);
	assert.equal(fixture.calls.some((call) => call.arguments_[0] === "install"), false);
});
...
test("Windows source recovers a valid backup after a crash between publication renames before any build", async () => {
	...
	const live = join(packageRoot, ".gentle-ai", "v2.4.0");
...
test("Windows source publication rolls back a prior bundle when final directory swap fails", async () => {
	...
	const versionDirectory = join(packageRoot, ".gentle-ai", "v2.4.0");
...
test("Darwin/Linux signed bundles retain their four-field manifest and reusable compatibility", { skip: process.platform === "win32" }, async () => {
	...
	const manifestPath = join(packageRoot, ".gentle-ai", "v2.4.0", "integrity.json");
...
test("extractors use only absolute trusted system paths, never lifecycle PATH or SystemRoot", { skip: process.platform === "win32" }, () => {
...
test("installer promotes only the expected regular executable with executable POSIX mode", { skip: process.platform === "win32" }, async () => {
	...
	const binary = join(packageRoot, ".gentle-ai", "v2.4.0", "gentle-ai");
