...
// v2.7.0 digests pinned from the published release: archive digests verified
...
const EXPECTED_ASSETS = {
	"darwin/amd64": { name: "gentle-ai_2.7.0_darwin_amd64.tar.gz", sha256: "5edf1e38894d6b2c1d257583cf4da23e2471812ed2efd0de33cbeef608339609", binarySha256: "e7aa77701e3cfd8a382a756ff2f257ba1a94ab4699050b3c3ac8e6237eb53950" },
	"darwin/arm64": { name: "gentle-ai_2.7.0_darwin_arm64.tar.gz", sha256: "a9efa8cf182d853b7d0115efbc73a83465d435ec429f6465bea9181ea8022fde", binarySha256: "497932a73664a958cf46b0b131277da4a41a48f8d7da4f777d1157f8ffa9f646" },
	"linux/amd64": { name: "gentle-ai_2.7.0_linux_amd64.tar.gz", sha256: "ff765a183247fe31d4739ff6df3d6874740979987ea310de3856c8a6e85375e9", binarySha256: "8748b9f18c05a831692abc0ae560fdc50a4e4c8f492705edcce7f65e067a7b12" },
	"linux/arm64": { name: "gentle-ai_2.7.0_linux_arm64.tar.gz", sha256: "d8e114cd4d226f6c974813a49fce4d3e2028f4880b87e08f270eadf0a0607561", binarySha256: "65dd6bd7249d5eb7a2c948eebd0aee1bf2e85e1a2123e9b3782e978b167c3875" },
} as const;
...
test("release mapping selects only the supported official v2.7.0 assets and pinned digests", () => {
	assert.deepEqual(
	...
	assert.equal(resolveGentleAiReleaseAsset("linux", "x64").name, "gentle-ai_2.7.0_linux_amd64.tar.gz");
	assert.equal(resolveGentleAiReleaseAsset("darwin", "arm64").name, "gentle-ai_2.7.0_darwin_arm64.tar.gz");
	for (const asset of Object.values(GENTLE_AI_RELEASE_ASSETS)) {
		assert.match(asset.url, /^https:\/\/github\.com\/Gentleman-Programming\/gentle-ai\/releases\/download\/v2\.7\.0\//);
	}
});
...
test("raw release assets are admitted only under a prerelease pin", () => {
	...
	assert.throws(() => gentleAiAssetForm("gentle-ai_2.7.0_linux_amd64", "2.7.0"), /only admitted for a prerelease pin/);
...
test("release digests are all-or-none and install fails closed while any digest is pending", async () => {
	...
	if (pending.length === digests.length) {
		...
		assert.equal(existsSync(join(packageRoot, ".gentle-ai", "v2.7.0", "gentle-ai")), false);
	}
});
...
function windowsGoFixture(fixtureOptions: WindowsGoFixtureOptions = {}) {
	...
	const metadata = [
		...
		`\tmod\tgithub.com/gentleman-programming/gentle-ai/v2\tv2.7.0\t${GENTLE_AI_WINDOWS_SOURCE_MODULE_CHECKSUM}`,
	...
	const run = async (file: string, arguments_: string[], options: WindowsGoCall["options"]) => {
		...
		if (arguments_.length === 1 && arguments_[0] === "version") return { stdout: fixtureOptions.reportedVersion ?? "gentle-ai 2.7.0\n", stderr: "" };
...
test("win32 x64 and arm64 install the exact Go SumDB source tag without archive lookup", async () => {
	for (const [arch, goArchitecture] of [["x64", "amd64"], ["arm64", "arm64"]] as const) {
		...
		assert.deepEqual(fixture.calls.filter((call) => call.file === goPath).map((call) => call.arguments_.slice(0, 2)), [
			["version"], ["install", "github.com/gentleman-programming/gentle-ai/v2/cmd/gentle-ai@v2.7.0"], ["version", "-m"],
		]);
	}
});
...
test("Windows source install reports missing or too-old Go without publishing a binary", async () => {
	for (const fixtureOptions of [
		...
		assert.equal(existsSync(join(packageRoot, ".gentle-ai", "v2.7.0", "gentle-ai.exe")), false);
...
test("Windows source install cleans staging after Go failure or wrong built version", async () => {
	for (const fixtureOptions of [
		...
		const runtimeDirectory = join(packageRoot, ".gentle-ai", "v2.7.0");
...
test("Windows source installs reuse only a fully verified package-local binary", async () => {
	...
	await writeFile(join(packageRoot, ".gentle-ai", "v2.7.0", "integrity.json"), "{}\n");
...
async function hardenedWindowsGoFixture(packageRoot: string, fixtureOptions: HardenedGoFixtureOptions = {}) {
	...
	const run = async (file: string, arguments_: string[], options: HardenedGoCall["options"]) => {
		...
		if (arguments_.length === 1 && arguments_[0] === "version") return { stdout: "gentle-ai 2.7.0\n", stderr: "" };
		throw new Error(`unexpected command: ${file} ${arguments_.join(" ")}`);
	};
	return {
		calls,
...
test("Windows source installation resolves one validated Go executable and seals hostile Go environment", async () => {
	...
	assert.deepEqual(goCalls.map((call) => call.arguments_.slice(0, 2)), [
		["version"],
		["install", "github.com/gentleman-programming/gentle-ai/v2/cmd/gentle-ai@v2.7.0"],
		["version", "-m"],
	]);
...
test("Windows source manifest binds verified Go metadata and architecture", async () => {
	...
	const manifest = JSON.parse(await readFile(join(packageRoot, ".gentle-ai", "v2.7.0", "integrity.json"), "utf8")) as Record<string, string>;
	assert.equal(result.installed, true);
...
test("Windows source installation treats a fresh ownerless lock as active", async () => {
	...
	const lockPath = join(packageRoot, ".gentle-ai", ".v2.7.0.install.lock");
...
test("Windows source installation preserves a lock whose owner nonce changed before release", async () => {
	...
	const lockOwnerPath = join(packageRoot, ".gentle-ai", ".v2.7.0.install.lock", "owner.json");
	const fixture = await hardenedWindowsGoFixture(packageRoot);
...
function tombstonePath(packageRoot: string, nonce: string): string {
	return join(packageRoot, ".gentle-ai", `.v2.7.0.install.tombstone-${nonce}`);
}
...
async function tombstones(packageRoot: string): Promise<string[]> {
	...
	return existsSync(runtimeRoot)
		? (await readdir(runtimeRoot)).filter((entry) => entry.startsWith(".v2.7.0.install.tombstone-"))
		: [];
}

function backupBundlePath(packageRoot: string, nonce: string): string {
	return join(packageRoot, ".gentle-ai", `.v2.7.0.backup-${nonce}`);
}
...
test("Windows source acquisition fails closed when a tombstone appears after its first scan", async () => {
	...
	assert.equal(existsSync(join(packageRoot, ".gentle-ai", ".v2.7.0.install.lock")), false);
	assert.equal(fixture.calls.some((call) => call.arguments_[0] === "install"), false);
});
...
test("Windows source recovers a valid backup after a crash between publication renames before any build", async () => {
	...
	const live = join(packageRoot, ".gentle-ai", "v2.7.0");
...
test("Windows source publication rolls back a prior bundle when final directory swap fails", async () => {
	...
	const versionDirectory = join(packageRoot, ".gentle-ai", "v2.7.0");
...
test("Darwin/Linux signed bundles retain their four-field manifest and reusable compatibility", async () => {
	...
	const manifestPath = join(packageRoot, ".gentle-ai", "v2.7.0", "integrity.json");
...
test("a pinned asset falls back to its gentle-pi mirror only on download failure, and a stable pin declares no mirror", async () => {
	...
	const baseAsset = { name: "gentle-ai_2.7.0_linux_amd64.tar.gz", sha256: createHash("sha256").update(payload).digest("hex"), binarySha256: createHash("sha256").update(binary).digest("hex"), url: "https://example.invalid/upstream", mirrorUrl: "https://example.invalid/mirror", executable: "gentle-ai" };
...
test("installer promotes only the expected regular executable with executable POSIX mode", async () => {
	...
	const binary = join(packageRoot, ".gentle-ai", "v2.7.0", "gentle-ai");
