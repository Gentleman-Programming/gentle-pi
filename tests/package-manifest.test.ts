...
test("package verification binds the published Gentle AI v2.5.0-rc.3 runtime pin", () => {
	...
	assert.match(installer, /INSTALLER_VERSION = "2\.5\.0-rc\.3"/);
	assert.match(installer, /GENTLE_AI_WINDOWS_SOURCE_PACKAGE.*GENTLE_AI_WINDOWS_SOURCE_MODULE/);
	assert.match(installer, /GENTLE_AI_WINDOWS_SOURCE_MODULE_CHECKSUM = "h1:zDmIUCSZxFowQQjR4IS\+YhvHPKHABYKyRT3ytZi8Qj4="/);
	...
	assert.match(verifier, /v2\.5\.0-rc\.3/);
});
