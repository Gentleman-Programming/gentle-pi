...
A successful postinstall in Pi's **global npm-managed** `agent-home/npm/node_modules/gentle-pi` or exact **global Pi Git-managed** `agent-home/git/github.com/Gentleman-Programming/gentle-pi` installation persists `"tuiMode": "fullscreen"` in `agent-home/settings.json` only when `tuiMode` is not already set, preserving other settings. Agent home resolves through `GENTLE_PI_AGENT_HOME`, then `PI_CODING_AGENT_DIR`, then `~/.pi/agent`. An explicit mode choice — `"regular"` included — is never overwritten: use `/settings` to switch modes, and later recognized postinstalls preserve that choice. Existing project overrides still take precedence.
